        ...
        <div class="max-w-md mx-auto space-y-6">
                    ...
                    <template v-else>
                        <div v-if="canSignin" class="space-y-4">
                            ...
                            <p v-if="continuousDays > 0" class="text-sm text-orange-600">
                                连续签到 {{ continuousDays }} 天，再签到 {{ 7 - (continuousDays % 7) }} 天可获双倍奖励！
                            </p>
            ...
            <div class="card">
                ...
                <div class="card-body space-y-2 text-sm text-gray-600">
                    ...
                    <p>• 连续签到3天额外奖励 {{ continuousReward }} 积分</p>
                    <p>• 连续签到7天奖励翻倍</p>
                    <p>• 积分可抵扣订单金额</p>
                </div>
            </div>
                    ...
                    <div class="mt-4 space-y-2">
                        ...
                        <div v-else class="space-y-2">
                            <div v-for="log in logs.slice(0, 5)" :key="log.id" class="flex justify-between text-sm">
                                <span class="text-gray-600">{{ log.description || '积分变动' }}</span>
...
function loadPoints() {
    pointsLogs(1, 20).then((d) => {
        logs.value = (d && d.list) ? d.list : (Array.isArray(d) ? d : [])
    }).catch(() => {})
}

function loadBalance() {
    fetch('/api/v1/user/points').then(r => r.json()).then((d) => {
        if (d.code === 0) {
            balance.value = d.data?.balance || d.data?.points || 0
        }
    }).catch(() => {})
}
...
function doSignin() {
    ...
    signIn().then((d) => {
        const data = d || {}
        Toast.success(`签到成功！获得 ${data.points || signinReward.value} 积分`)
        canSignin.value = false
        continuousDays.value = data.continuous_days || (continuousDays.value + 1)
        balance.value += data.points || signinReward.value
        loadPoints()
    }).catch((e) => {
        if (e.message && e.message.includes('已签到')) {
            Toast.info('今日已签到')
